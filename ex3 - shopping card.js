...
function addProductToCart(productId) {
  ...
  if (item) {
    item.quantity += 1; // Increment quantity if already exists
  } else {
    SHOPPING_CART.push({ id: productId, quantity: 1 }); // Add new item
  }
...
function removeProductFromCart(productId) {
  ...
  if (itemIndex !== -1) {
    if (SHOPPING_CART[itemIndex].quantity > 1) {
      SHOPPING_CART[itemIndex].quantity -= 1; // Decrement quantity if more than 1
    } else {
      SHOPPING_CART.splice(itemIndex, 1); // Remove item if quantity is 1
    }
  }
}
